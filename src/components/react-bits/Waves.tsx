...
class Noise {
  ...
  perlin2(x: number, y: number): number {
    ...
    const iX = X & 255;
    const iY = Y & 255;
    const n00 = this.gradP[iX + this.perm[iY]].dot2(fx, fy);
    const n01 = this.gradP[iX + this.perm[iY + 1]].dot2(fx, fy - 1);
    const n10 = this.gradP[iX + 1 + this.perm[iY]].dot2(fx - 1, fy);
    const n11 = this.gradP[iX + 1 + this.perm[iY + 1]].dot2(fx - 1, fy - 1);
...
const Waves: React.FC<WavesProps> = ({
  ...
  useEffect(() => {
    ...
    // Observer to pause animation loop when element is off-screen (saves mobile battery & CPU/GPU)
    const observer = new IntersectionObserver(
      ([entry]) => {
        const isIntersecting = entry.isIntersecting;
        isVisibleRef.current = isIntersecting;
        if (isIntersecting && frameIdRef.current === null) {
          frameIdRef.current = requestAnimationFrame(tick);
        }
      },
  ...
  return (
    ...
    >
      {showCursorDot && (
        <div
          className="absolute top-0 left-0 bg-white/20 rounded-full w-[0.5rem] h-[0.5rem] pointer-events-none transition-opacity duration-300"
          style={{
