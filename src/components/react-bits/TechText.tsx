...
const TechText = ({
  ...
  useEffect(() => {
    ...
    const family = (s: Settings) => {
      if (s.fontFamily && !s.fontFamily.includes('var(')) return s.fontFamily;
    ...
    const ensureLayout = (s: Settings): Word => {
      ...
      const bbWidth = m.actualBoundingBoxLeft + m.actualBoundingBoxRight;
      const advWidth = m.width;
      const fitWidth = Math.max(bbWidth, advWidth);
      const fit = Math.min(
        1,
        (width * 0.9) / Math.max(fitWidth, 1),
      ...
      const inkWidth = Math.max(m.actualBoundingBoxLeft + m.actualBoundingBoxRight, m.width);
      ...
      const next = {
        ...
        right: x - m.actualBoundingBoxLeft + inkWidth,
