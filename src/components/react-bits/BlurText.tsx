...
const BlurText: React.FC<BlurTextProps> = ({
  ...
  const elements = useMemo(() => {
    if (Array.isArray(text)) {
      if (animateBy === 'words') {
        return text.flatMap(segment => 
          segment.text.split(' ').map(word => ({ char: word, className: segment.className }))
        );
      } else {
        return text.flatMap(segment => 
      ...
      const arr = animateBy === 'words' ? text.split(' ') : text.split('');
      return arr.map(char => ({ char, className: '' }));
    }
  }, [text, animateBy]);
